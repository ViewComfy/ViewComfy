...
import { SaveAsAppDialog } from '@/components/save-as-app-dialog';
...
export default function ViewComfyPage() {
    ...
    const [saveAsAppDialogOpen, setSaveAsAppDialogOpen] = useState(false);
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    if (parsed.file_type === "view_comfy") {
                        ...
                        // Fetch object_info from ComfyUI to get input options
                        let objectInfo;
                        try {
                            ...
                            console.warn('Failed to fetch object_info, dropdowns may not work:', err);
                        }

                        viewComfyStateDispatcher({
                            type: ActionType.SET_VIEW_COMFY_DRAFT,
                            payload: { viewComfyJSON: workflowAPItoViewComfy(parsed, objectInfo), workflowApiJSON: parsed, file }
                        });
                    }
                } catch (error) {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
                                ...
                                <div className="w-full flex flex-wrap items-center gap-4 mb-4 ml-1">
                                    ...
                                    {showDeleteWorkflowButton() && (
                                        <div className="flex gap-2">
                                            ...
                                            <Button
                                                variant="default"
                                                onClick={() => setSaveAsAppDialogOpen(true)}
                                            >
            ...
            <SaveAsAppDialog
                open={saveAsAppDialogOpen}
                onOpenChange={setSaveAsAppDialogOpen}
                workflowData={viewComfyState.currentViewComfy}
            />
        </div>
    )
}
