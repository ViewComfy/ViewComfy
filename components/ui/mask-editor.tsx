...
interface MaskEditorProps {
  ...
  onSave: (maskFile: File, compositeFile: File) => void;
  onCancel: () => void;
  className?: string;
}
...
export function MaskEditor({ imageUrl, existingMask, onSave, onCancel, className }: MaskEditorProps) {
  ...
  useEffect(() => {
    ...
    // Always use imageUrl (original image), not preview
    img.src = imageUrl;
  ...
  const handleSave = async () => {
    ...
    // INVERT alpha from mask
    for (let i = 0; i < imageData.data.length; i += 4) {
      const maskValue = maskData.data[i]; // white=255, black=0
      imageData.data[i + 3] = 255 - maskValue; // white→0, black→255
    }
    ...
    // For ComfyUI: Image with inverted alpha channel
    const maskFile = new File([imageWithAlphaBlob], 'masked_image.png', { type: 'image/png' });
    // For preview only
    const compositeFile = new File([previewBlob], 'preview.png', { type: 'image/png' });

    onSave(maskFile, compositeFile);
  };
